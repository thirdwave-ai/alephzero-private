...
procmaps_iterator* pmparser_parse(int pid){
	...
	while( !feof(file) ){
		if (fgets(buf,PROCMAPS_LINE_MAX_LENGTH,file) == NULL){
			if (errno == EAGAIN) {
				continue;
			} else if (errno || ferror(file)) {
				fprintf(stderr,"pmparser : fgets failed, (%d){%d} %s\n", errno, ferror(file), strerror(errno));
				...
				// Just got no bytes. Probably eof. Next while loop iter will catch this.
				break;
			}
		}
		//allocate a node
...
void pmparser_free(procmaps_iterator* p_procmaps_it){
	if (!p_procmaps_it) { return; }
	procmaps_struct* act = p_procmaps_it->head;
	while(act!=NULL) {
