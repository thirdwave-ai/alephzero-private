...
// "Unknown" if the address is unknown in the memory map, or
// "" if the address is not backed by a mmapped file-like object.
//
...
char* a0_reverse_map_arena_to_path(a0_transport_hdr_t* arena) {
  ...
  // Fall-through on not-found will have pathname == "Unknown".
